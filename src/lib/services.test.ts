...
describe("full groom coat-type pricing", () => {
  ...
  it("uses the current Full Groom starting prices", () => {
    const groom = getServiceById("custom-full-haircut");
    assert.ok(groom);
    assert.equal(getTierForPet(groom, 20)?.priceFrom, 170);
    assert.equal(getTierForPet(groom, 40)?.priceFrom, 190);
    assert.equal(getServicePriceEstimate(groom, 12)?.from, 150);
    assert.equal(getServicePriceEstimate(groom, 20)?.from, 170);
    assert.equal(getServicePriceEstimate(groom, 40)?.from, 190);
    assert.notEqual(getServicePriceEstimate(groom, 12)?.from, 140);
    assert.notEqual(getServicePriceEstimate(groom, 20)?.from, 160);
    assert.notEqual(getServicePriceEstimate(groom, 40)?.from, 180);
  });
});
