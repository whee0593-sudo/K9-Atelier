...
- **Privacy gate can be active in local dev.** When `content/business.json` sets `site.privacyMode: true` and `SITE_ACCESS_PASSWORD` is configured, `middleware.ts` redirects every non-public page to `/under-construction`. To view the real site you must unlock it:
  ...
  - Secrets and the private mobile-service base address are never stored in `business.json`. Put `SITE_ACCESS_PASSWORD`, `SITE_PREVIEW_SHARE_TOKEN`, `SITE_BASE_ADDRESS`, and `GOOGLE_MAPS_API_KEY` in `.env.local`; `.env*.local` is gitignored.
  - Travel quotes use Google Geocoding + Routes when `GOOGLE_MAPS_API_KEY` is set (server-side only). If the key is missing, or Google returns `REQUEST_DENIED` / similar config errors, `src/lib/geo.ts` falls back to Nominatim + OSRM. A Google `ZERO_RESULTS` address is not retried on OSM. Setup: `content/MAPS.md`.
